    ...
    public boolean saveDefault(final String resourceName) throws ResourceLoaderException {
        if (!Files.exists(this.dataDirectory)) {
            try {
                ...
                throw new ResourceLoaderException(ResourceLoaderException.Reason.DIRECTORY_NOT_CREATED, resourceName, exception);
            }
        }
        ...
        if (!Files.exists(resourcePath)) {
            try (final InputStream in = this.loadingClass.getClassLoader().getResourceAsStream(resourceName)) {
                ...
                throw new ResourceLoaderException(ResourceLoaderException.Reason.DEFAULT_FILE_NOT_SAVED, resourceName, exception);
            }
        }
